PathfinderHelpModule = Object:extend()
...
function HelpModule:new(console, y)
    ...
    self.console:addLine(0.16, '    rewrite       Displays classes window') 
    self.console:addLine(0.18, '    clear         Clear saved data options') 
    self.console:addLine(0.20, '    security      Configure PATHFINDER security settings') 
    self.console:addLine(0.22, "    passives      Displays passive skill tree window") 
    self.console:addLine(0.24, "    connect       Initiates PATHFINDER") 
    ...
    self.console:addLine(0.32, '    credits       Displays PATHFINDER credits') 
    self.console:addLine(0.34, '    return        Return to BYTEPATH termnial') 
...
function HelpModule:update(dt)
    ...
    if input:pressed('return') then
        ...
        elseif self.selection_index == 3 then
            table.insert(self.console.modules, PathfinderSecurityModule(self.console, self.console.line_y))
        elseif self.selection_index == 4 then
            gotoRoom('SkillTree')
        elseif self.selection_index == 5 then
            gotoRoom('PathfinderConnect')
