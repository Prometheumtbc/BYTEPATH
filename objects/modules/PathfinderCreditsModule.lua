PathfinderCreditsModule = Object:extend()

function PathfinderCreditsModule:new(console, y)
    self.console = console
    self.y = y
    
    self.console:addLine(0.02, '')
    self.console:addLine(0.04, '    PATHFINDER by $Prometheum%')
    self.console:addLine(0.06, '    BYTEPATH by $a327ex%')
    self.console:addLine(0.08, '    music by $Kubbi%')
    self.console:addLine(0.10, '    sound effects from $freesound.org% by')
    self.console:addLine(0.12, '        $fins jeckkech josepharaoh99 KorGround NSStudios TreasureSounds TheDweebMan%')
    self.console:addLine(0.14, '        $LittleRobotSoundFactory GameAudio NenadSimic DrMinky pyzaist CGEffex broumbroum%')
    self.console:addLine(0.16, '    fonts by')
    self.console:addLine(0.18, '        $Daniel Linssen Troy Mark Simonson%')
    self.console:addLine(0.20, '    made with $love2d.org% with help from')
    self.console:addLine(0.22, '        $slime bartbes CapsAdmin rxi RolandYonaba vrld pelevesque davisdude gvx bakpakin%')
...
function PathfinderCreditsModule:update(dt)
...
function PathfinderCreditsModule:draw()
    if not self.active then return end
...
function PathfinderCreditsModule:setSelections()
    local font = self.console.font
    self.selections = {
        ...
        [8] = {{ox = font:getWidth('    made with '), w = font:getWidth('love2d.org'), link = 'https://love2d.org/'}},
        [9] = {
